 ...
 *****************************************************************************/
static WE_BOOL isLink(const SmtrSchemesTable *sch, char *str, SmtrScheme *scheme)
{
    int i;
    WE_BOOL valid = TRUE;
    char *s = str;
    ...
    for (i = 0; SMTR_SCHEME_NONE != sch[i].scheme; i++)
    {
        *scheme = sch[i].scheme;
        /* Special handling for mail and phone number */
        if (SMTR_SCHEME_MAIL == sch[i].scheme) 
        {
        ...
        else if (SMTR_SCHEME_PHONE == sch[i].scheme) 
        {
            if (strlen(str) > 2)
            {
                valid = TRUE;
 ...
 *****************************************************************************/
static WE_BOOL doParse(char *str, SmtrParseResult **result)
{
    char *oldPos;
    char *endPos;
    char tmpChar;
    ...
    while(str[0] != '\0')
    {
        ...
        *endPos = '\0';
        ...
        if (isLink(schemes, str, &scheme)) 
        {
            ...
            smtrInsertParseEntry(utf8Pos, str, utf8Len, scheme, result);
        }
        utf8Pos += utf8Len;
 ...
 *****************************************************************************/
WE_BOOL smtrParse(SmtrInstance *inst)
{
    char *str;
