...
#define SMTR_BASELINE_SIZE        (1)

#define SMTR_LINK_COLOR {0, 0, 255}
    ...
    const WeSize *size)
{
    ...
    if (0 == (imgHandle = WE_WIDGET_IMAGE_CREATE_EMPTY(WE_MODID_SMTR, 
        (WeSize *)size, style)))
    {
        return 0;
    }
 ...
 *****************************************************************************/
static WE_BOOL createIndexTable(SmtrInstance *inst, WeFont *currFont)
{
	...
	if(currFont){
		style = WE_WIDGET_STYLE_CREATE(WE_MODID_SMTR, &inst->prop.textColor, 
    ...
    vertResize = IS_SET(inst->prop.flags, SMTR_PROP_VERTICAL_RESIZE);
    if (vertResize) 
    {
        tmpSize.height = 0; 
    }
    ...
    utf8Str = smtrGetStringBufferFromHandle(strHandle);    
    ...
    while (trav.continue_at && *trav.continue_at) 
    {
        ...
        if (trav.fits) 
        {
            smtrAddRowIndex(inst, (WE_UINT16)trav.start_idx, (WE_UINT16)trav.width);
        }
        else
        {
            ...
            for (i = 0; i < lines; i++)
            {
                ...
                smtrAddRowIndex(inst, (WE_UINT16)index, (WE_UINT16)trav.width);
                index += visibleChars;
            }
        }
    }
 ...
 * \return success, otherwise -1.
 *****************************************************************************/
static WE_BOOL calculateImageSize(const SmtrInstance *inst, const WeSize *maxSize,
    WeSize *size, WE_INT16 *rowHeight, WeFont *currFont)
{
    ...
    /* Check flags */
    vertResize = IS_SET(inst->prop.flags, SMTR_PROP_VERTICAL_RESIZE);
    horizResize = IS_SET(inst->prop.flags, SMTR_PROP_HORIZONTAL_RESIZE);
    strHandle = inst->stringHandle;
    ...
    *rowHeight = (WE_INT16)(fontHeight + SMTR_BASELINE_SIZE);
    ...
    const WeSize *size, WE_INT16 rowHeight, WeStyleHandle style)
{
    ...
    SmtrIndexTableEntry *p;
    char *charPos = NULL;
	int align = msaGetAlignment();
 ...
 *****************************************************************************/
static WE_BOOL isVisible(const SmtrParseResult *link, int start, 
    const SmtrInstance *inst)
{
 ...
 * \return The new instance or NULL if no new instance could be created.
 *****************************************************************************/
WE_BOOL smtrRender(SmtrInstance *inst)
{
    WeSize *size;
    SmtrParseResult *tmp;
    WeImageHandle imgHandle;
    ...
    WE_INT16 rowHeight;
    WeStyleHandle style;
    /*TR 17667, here we can give a new font to WeStyle to test render when the
	 * image has different style than the string.
	 */
	WeFont fonttmp = {0};
    ...
    if (0 == (style = WE_WIDGET_STYLE_CREATE(WE_MODID_SMTR, &inst->prop.textColor, 
    ...
    else
    {
        ...
        if (0 == (inst->gadgetHandle = 
            WE_WIDGET_IMAGE_GADGET_CREATE(WE_MODID_SMTR, imgHandle, size, 
            WeImageZoomAutofit, 
            0, style)))
        {
            (void)WE_WIDGET_RELEASE(style);
