...
#define VSNPRINTF                   GET_HELPER()->vsnprintf
...
#define ERR_MALLOCREC_EX(t,n,pp)  (*(pp)=(t *)0,ERR_REALLOC(sizeof(t)+n,(void **)pp))
...
invoke the function MEMCPY, there is no need for an interface pointer. Applications
can directly invoke MEMCPY().  Some of the functions provided here make
direct calls to the standard C library functions (ex: MEMCPY() directly invokes the
C library function memcpy()). However, these functions are provided here for two
reasons: 
   ...
   direct calls to the standard C library functions (there by, avoding the app from 
   being associated with the baggage of static C library)
   ...
   such as memcpy(), they need to invoke the corresponding header files (ex: memory.h)
   and these header files do contain static data in them. Hence, it would
   prevent the app from being dynamic loadable.
...
The function will then mark the block as available to be moved in memory.  Note that
this function monitors a "lock count" on the memory block.  This allows the caller
to pair LOCKMEM and UNLOCKMEM inside sub-routines that may be called 
while the memory is locked.  In such cases, the memory will not actually 
...
As indicated, this function accepts a pointer to a handle of allocated memory.
The function will then mark the block as unavailable to be moved in memory.  Note that
this function also increments a "lock count" on the memory block.  This allows
the caller to pair LOCKMEM and UNLOCKMEM inside sub-routines that may be
...
Prototype:
   ...
   boolean STRBENDS( const char *pszSuffix, const char *psz);
...
Description: 
   This function compares the two strings, s1 and s2, lexicographically. It returns
   an integer value that indicates the comparison result. Note that only in this function
...
This function does not support %f in the format string. If %f is found anywhere within 
the format string, this function fills a null character in the starting location of the 
destination string and returns immediately without doing any processing.
...
to the documentation of the standard C library function sprintf() on the
corresponding platform (Windows / ARM)
...
Prototype:
   ...
   void STREXPAND(const char *pSrc, int nCount, byte *pDest, int nSize)
...
Parameters:
   pszDest: On input, this is a pointer to NULL terminated source string. On return, this buffer 
...
On Windows, the native format is same as the windows bitmap format.

Typical usage of this function: If the user has raw data buffer containing
a windows bitmap, it can be passed to this function to convert it into
a native format so that the IDISPLAY_BITBLT() function can be used on this
format to display the image. Please note that SDK does not support 16 bit and 24 bit
format BMP.

Note:  If the pbReallocated is returned as TRUE, then the image was 
reallocated.  In order to free this data, the caller must use the SYSFREE()
call.
...
Parameters:
   pSrcBuffer: [in]: Pointer to a buffer containing the windows bitmap. One of the ways
   of constructing this is to read the contents of a windows .bmp file into
   a memory buffer. This buffer is converted to native format on return.

   pii: [out]: 
   On return, the AEEImageInfo struct pointed to by this member contains
   valid information about the converted image (such as, width, height, color
   depth, etc).

   pbRealloc: [out]: 
   On return, this is set to TRUE or FALSE depending on whether or not the CONVERTBMP()
   function did separate memory allocation for the buffer that is returned
   from this function. If this is set to TRUE, the caller must release the buffer
   returned from this function using SYSFREE() after the buffer is used.
...
Return Value:
   ...
   is set to true on return, the caller must release this buffer using IHEAP_FREE()
   after using the buffer.  
   On Windows, since the native format is the same as the bmp format, the pointer
   returned from this function will be identical to pSrcBuffer. Also, bRealloc will 
   be FALSE on Windows since no new allocation is done. The pii parameter, on
   return, will contain information about the image (such as, width, height, 
   color depth, etc).
...
Parameters:
   ...
   otherwise it is ignored. It specifies the number of digits that must be present
   in the final string. For ex: if n is set to 245 and nPad is set to 5, then the 
   output string shall contain 00245. 
   nRemaining: [in/out]: On input, this pointer points to an integer that specifies the 
   size of pszBuf. This is specified in terms of total number of characters (AECHARs)
   including the padding, that can be written to the string. 
   On output, this points to an integer that denotes the unused space (in terms of
   number of ARCHARs) left in pszBuf. The difference between the output and input values 
   denotes the number of characters written to pszBuf (excluding the null character).

Return Value:
   If successful, returns a pointer to the beginining of the unused portion of pszBuf. This
   is the portion in pszBuf after the formatted wide string has been written.
...
Parameters:
   ...
   lenSource: [in]: the maximum string length to copy into pDest. It does not need
   to include null character. If it is set to -1, then the entire pSrc is copied to
   pDest
...
Description: 
   ...
   be null-terminated. If the first byte of a character is greater than 0x7f,it is
   treated as a 2-byte character and the two bytes will be counted has one
   single character.
...
Parameters:
   ...
   number shall be placed. It can be of the form "1.0.0.15" or "1.0.0.15(MSM3100)" depending
   on whether or not the MSM information is requested.

   nsize [in]: size of the pszString buffer in bytes
   wFlags [in]: Specifies the type of information to retrieve. It can be a combination of 
...
Description: 
This function returns the local timezone offset from UTC, in seconds. Optionally returns
...
is just for controlling display of a timezone name if desired).The returned value is 
added to UTC to give local time, or subtracted from local time to give UTC time.

  UTC = local time - bias 
(where, bias is the value returned from this function)
...
Parameters:
   pbDaylightSavings: [in/out]: If non NULL on input, this flag specifies whether or
   not Daylight savings time is active on return
...
Description: 
   This method returns the current time of day in milliseconds. 
Note that the value returned by this call depends on the device's current
time of day value.  This value is obtained from the base station.  As such
this value may change dramatically when the phone first acquires system 
coverage.
...
Return Value: 
   32-bit millisecond value since midnight.
...
Description: 
   This method returns the millisecond timer from the time
the device was powered.  Note that unlike GetTimeMS, this value will not
change dramatically due to initial acquisition of system coverage.  
...
Return Value: 
   32-bit millisecond value from the time the device was powered.
...
Description: 
   This method returns the number of seconds, adjusted for 
local time, since midnight Jan 6, 1980 GMT.
...
Return Value: 
   32-bit number of seconds, adjusted for local time, since 1/6/80 00:00:00 GMT
...
Description: 
   This method returns the number of seconds since midnight Jan 6, 1980 GMT.
...
Return Value: 
   32-bit number of seconds since 1/6/80 00:00:00 GMT
...
Description: 
   ...
   value.  This value represents the number of seconds since Jan 6 1980,
   the phone reference time. If the input value is 0, the current system 
   time is used.
...
Parameters:
   dwSecs: Seconds since Jan 6 1980, the phone reference time
...
Comments:  
   ...
   difference between phone reference time Jan 6, 1980 and PC 
   reference time Jan 1, 1970.
...
Prototype:
   ...
   boolean INET_ATON(const char *psz, unsigned long *pulAddr);

Parameters:
   const char *psz: string containing the addr (like "1.2.3.4")
   unsigned long *pulAddr: pointer to the inet address to be filled
...
Prototype:
   ...
   void INET_NTOA(unsigned long ulAddr, char *pc, int nLen);

Parameters:
   unsigned long ulAddr: the inet address to converted
...
Description: 
    HTONL()and NTOHL() convert a host byte order unsigned long to network byte order, and
   vice-versa
...
Parameters:
   unsigned long ul: the long to convert
...
Description: 
   The functions HTOLEL() and  LETOHL() convert a host byte order unsigned long to little-endian byte order, and
   vice-versa.

Prototype:

   unsigned long HTOLEL(unsigned long ul);

   unsigned long LETOHL(unsigned long ul);
