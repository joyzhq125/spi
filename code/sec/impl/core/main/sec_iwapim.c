                        ...
                        const WE_UCHAR * pucRandval)
{
    ...
    switch (SEC_STATES)
    {
        ...
        case E_STATE_GET_WTLS_KEY_EXCHANGE: 
            ...
            SEC_WANTS_TO_RUN = 0; 
            Sec_iWimWtlsKeyExchange(hSecHandle,iTargetID, stParam,  pucRandval,ucHashAlg);
            SEC_STATES++;
            break;
        case E_STATE_GET_WTLS_KEY_EXCHANGE_RESP:
            ...
            if (!SEC_CUR_WIM_RESP)
            {
                Sec_WtlsKeyExchangeResp(hSecHandle, iTargetID, M_SEC_ERR_INSUFFICIENT_MEMORY, 
                                        0, NULL, 0);
            ...
            {
                Sec_WtlsKeyExchangeResp(hSecHandle, iTargetID, M_SEC_ERR_GENERAL_ERROR, 
                                        0, NULL, 0);
                Sec_DpHandleCleanUp( hSecHandle );
            ...
            {
                Sec_WtlsKeyExchangeResp(hSecHandle, pstWimStKeyexchResp->iTargetID, pstWimStKeyexchResp->iResult, 
        ...
        case E_STATE_GET_WTLS_KEY_EXCHANGE_AGAIN:
            {
                ...
                switch(iStatus)
                {
                    ...
                    default:                    
                        Sec_WtlsKeyExchangeResp(hSecHandle, iTargetID, iStatus, 0, NULL, 0);
                        Sec_DpHandleCleanUp( hSecHandle );
                        break;
                }
            }
                        ...
                        const WE_UCHAR * pucSeed, WE_INT32 iSeedLen, WE_INT32 iOutputLen)
{
    ...
    switch (SEC_STATES)
    {
        ...
        case E_STATE_WTLS_GET_PRF_RESULT:    
            SEC_WAITING_FOR_WIM_RESULT = 1;
            SEC_WANTS_TO_RUN = 0;
            Sec_iWimWtlsGetPrfResult(hSecHandle, SEC_DEL_ID_FLAG(iTargetID),  iMasterSecretId, 
                                    pucSecret, iSecretLen,  pcLabel, pucSeed, iSeedLen, 
                                    iOutputLen, (WE_UINT8)M_SEC_WTLS_CONNECTION_MODE,ucAlg);
            SEC_STATES++;
            break;
        case E_STATE_WTLS_GET_PRF_RESULT_RESP:
            ...
            if (!SEC_CUR_WIM_RESP)
            {
                Sec_WtlsGetPrfResultResp(hSecHandle, iTargetID, 
            ...
            {
                Sec_WtlsGetPrfResultResp(hSecHandle, iTargetID, M_SEC_ERR_GENERAL_ERROR, NULL, 0);
                Sec_DpHandleCleanUp( hSecHandle );
                Sec_TlHandleWimStructFree(SEC_CUR_WIM_RESP);
                SEC_CUR_WIM_RESP = NULL;
                return;
            ...
            {
                Sec_WtlsGetPrfResultResp(hSecHandle, iTargetID, pstSecWimStPrfResp->iResult, 
        ...
        case E_STATE_WTLS_GET_PRF_RESULT_AGAIN:
            {
                ...
                switch(iStatus)
                {
                    ...
                    case M_SEC_ERR_MISSING_KEY:
                        ...
                        SEC_WAITING_FOR_WIM_RESULT = 1;
                        SEC_WANTS_TO_RUN = 0;
                        Sec_iWimWtlsGetPrfResult(hSecHandle, SEC_DEL_ID_FLAG(iTargetID), 
                    ...
                    case M_SEC_ERR_USER_NOT_VERIFIED:
                        ...
                        //lint -fallthrough
                    default:                              
                        Sec_WtlsGetPrfResultResp(hSecHandle, iTargetID, iStatus, NULL, 0);
                        Sec_DpHandleCleanUp( hSecHandle );
                        break;
                }
            }
                          ...
                          const WE_UCHAR * pucBuf, WE_INT32 iBufLen)
{
    ...
    switch (SEC_STATES)
    {
        ...
        case E_STATE_WTLS_COMPUTE_SIGN: 
            SEC_WAITING_FOR_WIM_RESULT = 1;
            SEC_WANTS_TO_RUN = 0;
            Sec_iWimWtlsCalSign(hSecHandle, iTargetID, pucKeyId, iKeyIdLen, 
                                        pucBuf, iBufLen, (WE_UINT8)M_SEC_WTLS_RSA_SIGN_CERT);
            SEC_STATES++;
            break;
                ...
                switch(pstWimStCertNamesConnResp->iNbrOfCerts)
                {
                    ...
                    case E_STATE_RESP_GET_USER_CERT_AND_KEY_ID_AGAIN:                        
                        ...
                        Sec_iWimGetKeyIdAndUCert(hSecHandle, iCertId, 
                                                    (WE_UINT8)M_SEC_WTLS_CONNECTION_MODE);
                        SEC_STATES+=3;    
                        break;
                    default:                        
                        SEC_WAITING_FOR_UE_RESULT = 1;
