    ...
    switch (((St_SecDpParameter*)pvPara)->eType)
    {
        ...
        case E_SEC_DP_XCH_WTLS_KEY: 
        {
            ...
            if(pstData->pstStrct)
            {
                ...
                if ((E_SEC_WTLS_KEYEXCHANGE_RSA_ANON == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite) ||
                (E_SEC_WTLS_KEYEXCHANGE_RSA_ANON_512 == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite) ||
                (E_SEC_WTLS_KEYEXCHANGE_RSA_ANON_768 == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite))
                {
                    if ((pstData->pstStrct->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.usExpLen) > 0)
                    {
                        M_SEC_SELF_FREE(pstData->pstStrct->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.pucExponent);
                    }    
                    if ((pstData->pstStrct->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.usModLen) > 0)
                    {
                        M_SEC_SELF_FREE(pstData->pstStrct->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.pucModulus);
                    }    
                    if (255 == pstData->pstStrct->stParam.stParams.stKeyParam.stParamSpecifier.ucParameterIndex)                    
                    {
                        if ((pstData->pstStrct->stParam.stParams.stKeyParam.stParamSpecifier.usParamLen) > 0)
                        {
                            M_SEC_SELF_FREE(pstData->pstStrct->stParam.stParams.stKeyParam.stParamSpecifier.pucParams);
                        }    
                    }
                }
                else if ((E_SEC_WTLS_KEYEXCHANGE_RSA == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite) ||
                (E_SEC_WTLS_KEYEXCHANGE_RSA_512 == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite) ||
                (E_SEC_WTLS_KEYEXCHANGE_RSA_768 == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite))
                {
                    if ((pstData->pstStrct->stParam.stParams.stCertificates.usBufLen) > 0)
                    {
                        M_SEC_SELF_FREE(pstData->pstStrct->stParam.stParams.stCertificates.pucBuf);
                    }    
                }/* add by Sam [070126] */
                else if (E_SEC_WTLS_KEYEXCHANGE_SHARED_SECRET == (E_WTLSKeyExchMethods)pstData->pstStrct->stParam.ucKeyExchangeSuite)
                {
                    if ((pstData->pstStrct->stParam.stParams.stSecretKey.usIdLen) > 0)
                    {
                        M_SEC_SELF_FREE(pstData->pstStrct->stParam.stParams.stSecretKey.pucIdentifier);
                    }
                }
                M_SEC_SELF_FREE(pstData->pstStrct);
            }
...
void Sec_FreeMsg (WE_UINT8 ucID, WE_INT32 iMsg, void* pvData)
{
    ...
    switch (iMsg)
    {
        ...
        case M_SEC_DP_MSG_EXCHANGE_WTLS_KEY:
         {
            ...
            if ((E_SEC_WTLS_KEYEXCHANGE_RSA_ANON == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite) ||
            (E_SEC_WTLS_KEYEXCHANGE_RSA_ANON_512 == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite) ||
            (E_SEC_WTLS_KEYEXCHANGE_RSA_ANON_768 == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite))
            {
                if ((pst->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.usExpLen) > 0)
                {
                    M_SEC_SELF_FREE(pst->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.pucExponent);
                }    
                if ((pst->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.usModLen) > 0)
                {
                    M_SEC_SELF_FREE(pst->stParam.stParams.stKeyParam.stPubKey.stPubKey.stRsaKey.pucModulus);
                }    
                if (255 == pst->stParam.stParams.stKeyParam.stParamSpecifier.ucParameterIndex)                    
                {
                    if ((pst->stParam.stParams.stKeyParam.stParamSpecifier.usParamLen) > 0)
                    {
                        M_SEC_SELF_FREE(pst->stParam.stParams.stKeyParam.stParamSpecifier.pucParams);
                    }    
                }
            }
            else if ((E_SEC_WTLS_KEYEXCHANGE_RSA == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite) ||
            (E_SEC_WTLS_KEYEXCHANGE_RSA_512 == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite) ||
            (E_SEC_WTLS_KEYEXCHANGE_RSA_768 == (E_WTLSKeyExchMethods)pst->stParam.ucKeyExchangeSuite))
            {
                if ((pst->stParam.stParams.stCertificates.usBufLen) > 0)
                {
                    M_SEC_SELF_FREE(pst->stParam.stParams.stCertificates.pucBuf);
                }    
            }
        }
        break;
        ...
        {
            St_SecWtlsGetPrfResult* pst = (St_SecWtlsGetPrfResult*)pvData;
