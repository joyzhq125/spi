...
static WE_INT32 Sec_UeEncode(WE_VOID** ppvBuf,WE_INT32 iEvent,WE_VOID* pvData);
...
{ 
    ...
    {
        /*encode */
        Sec_UeEncode(&pvMsg,eSecEvt,pvData);
        M_SEC_UECALLBACKDATA = (WE_VOID*)pvMsg;
...
static WE_INT32 Sec_UeEncode(WE_VOID** ppvBuf,WE_INT32 iEvent,WE_VOID* pvData)
{
    ...
    {
        case M_UE_CONFIRM_EVENT:/*St_Confirm*/
            Sec_EncodeConfirm(ppvBuf, iEvent, pvData);
            break;
        case M_UE_CHANGE_PIN_EVENT:
            Sec_EncodeChangePin(ppvBuf, iEvent, pvData);
            break;
        case M_UE_SELECT_CET_EVENT:
            Sec_EncodeSelCert(ppvBuf, iEvent, pvData);
            break;        
        case M_UE_NAME_CONFIRM_EVENT:
            Sec_EncodeNameConfirm(ppvBuf, iEvent, pvData);
            break;           
        case M_UE_SIGNTEXT_CONFIRM_EVENT:
            Sec_EncodeSigntextConfirm(ppvBuf, iEvent, pvData);
            break;
        case M_UE_CREATE_PIN_EVENT :/*St_CreatePin*/
            Sec_EncodeCreatePin(ppvBuf, iEvent, pvData);
            break;
        case M_UE_HASH_EVENT:
            Sec_EncodeHash(ppvBuf, iEvent, pvData);
            break;
        case M_UE_PIN_EVENT:/*St_Pin*/
            Sec_EncodePin(ppvBuf, iEvent, pvData);
            break;
        case M_UE_SHOW_CERT_CONTENT_EVENT:
            Sec_EncodeShowCertContent(ppvBuf, iEvent, pvData);
            break;
        case M_UE_SHOW_CERT_LIST_EVENT :
            Sec_EncodeShowCertList(ppvBuf, iEvent, pvData);
            break;
        case M_UE_SHOW_SESSION_CONTENT_EVENT:
            Sec_EncodeShowSessionContent(ppvBuf, iEvent, pvData);
            break;
        case M_UE_SHOW_CONTRACT_LIST_EVENT :
            Sec_EncodeShowContractList(ppvBuf, iEvent, pvData);
            break;
        case M_UE_STORE_CERT_EVENT:
            Sec_EncodeStoreCert(ppvBuf, iEvent, pvData);
            break;
        case M_UE_WARNING_EVENT:/*St_Warning*/
            Sec_EncodeWarning(ppvBuf, iEvent, pvData);
            break;
        case M_UE_STORE_CONTRACT_EVENT:
            Sec_EncodeStoreContract(ppvBuf, iEvent, pvData);
            break;
        case G_Evt_SHOW_DLG:
            {
                St_ShowDlg* pstData = (St_ShowDlg*)pvData;
