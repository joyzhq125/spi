...
/*add by tang 070126*/
#define    SEC_ROOT_CERT     0x03
#define    SEC_CA_CERT       0x02
#define    SEC_CERT_OK       0x01
#define    M_SEC_FREE(p)     { WE_FREE(p);p = NULL;}
...
typedef struct tagSt_SecWimTlsCaCertParamList 
{
    WE_UINT32                   uiFileId;
    WE_INT32                   iIsRootCert;
    WE_UINT8                   *pucSubject;
    WE_UINT8                   *pucIssuer;
    WE_UINT8                   aucSubjectHash[20];
    WE_UINT16                  usIssuerLen;
    WE_UINT16                  usSubjectLen;
    WE_INT32                   iIsX509OK;
} St_SecWimTlsCaCertParamList;
#endif
...
#ifdef M_SEC_CFG_TLS
static WE_INT32 Sec_iWimGetTLSCaCertParamList(WE_HANDLE hSecHandle,
                                          St_SecWimTlsCaCertParamList **ppstCaCertParamList,
                                          WE_UINT8 *pucNumCaCert);
static WE_INT32 Sec_iWimTLSFindCaDNHashToUCertDNHash(WE_HANDLE hSecHandle,WE_INT32 iFileId,
                                                    const WE_UCHAR *pucCaDNHash, WE_INT32 iCaDNHashLen,
                                                    const WE_UCHAR *pucUCertIdxFile,WE_INT32 iCertIdxFileLen,
                                                    WE_INT32 *piFind);
WE_INT32 Sec_WimTlsGetCertNameByID(WE_HANDLE hSecHandle,    
                                   const WE_UCHAR *pucCertIdxFile,
                                   WE_INT32 iCertId, WE_UINT8 **ppucCertNames, 
                                   WE_UINT16 *pusNameLen, WE_INT16 *psCharSet,
                                   WE_INT8 *pcNameType);
#endif
                              ...
                              WE_INT32 iOutputLen, WE_UINT8 ucConnectionType,WE_UINT8 ucAlg)
{
    ...
    if (!Sec_WimIsInited(hSecHandle))
    {
        Sec_WimGetPrfResultResponse(hSecHandle, iTargetID, M_SEC_ERR_WIM_NOT_INITIALISED, NULL, 0);
        return;
    }
    /* ioutputLen maybe 0. */
    if (0 == iOutputLen)
    ...
    else if(ucConnectionType == M_SEC_TLS_CONNECTION_TYPE)
    {
                              ...
                              WE_UINT8 ucConnectionType)
{
    ...
    if ((1 == ucFound) && (pucTempCert != NULL)) /* find out the certificate.*/
    {
        ...
        {
            if(M_SEC_WTLS_CONNECTION_TYPE==ucConnectionType) 
            {
            ...
            else if(M_SEC_TLS_CONNECTION_TYPE==ucConnectionType)
            {
                pucRealCert = (WE_UCHAR*)WE_MALLOC((usTempCertLen + 2) * sizeof(WE_UCHAR)); 
                if (NULL == pucRealCert)
                {
                ...
                for (iIndexJ=0; iIndexJ<(WE_INT32)(usRealCertLen); iIndexJ++)
                {
                    pucRealCert[iIndexJ] = pucTempCert[iIndexJ+1];
                } 
            }
        }
    ...
    {
            Sec_WimGetUserCertAndKeyIdResponse(hSecHandle, M_SEC_ERR_OK, pucRealCert,\
                 usRealCertLen , pucKeyExId, iKeyExIdLen);
    }      
    ...
    else if ((*pucCert) == M_SEC_CERTIFICATE_X509_TYPE)   
    {
        ...
        if (iRes == M_SEC_ERR_OK)
        {
            iRes = Sec_WimChkX509Cert(hSecHandle, iTargetID, 
                ...
                pucCert+1, M_SEC_WTLS_CONNECTION_TYPE, 1, M_SEC_CERT_USAGE_ROOT);
        }
    }
    else
    {
...
{ 
    ...
    if (!Sec_WimIsInited(hSecHandle))
    {
        Sec_WimKeyExchangeResponse(hSecHandle, iTargetID, M_SEC_ERR_WIM_NOT_INITIALISED,
            0, NULL, 0);
        return;
    }
...
/*=====================================================================================
FUNCTION: 
        Sec_iWimGetTLSUserCertNames
CREATE DATE: 
        2007-1-19
AUTHOR: 
        tang
DESCRIPTION:
        Get tls user certificate's name.          
ARGUMENTS PASSED:
        hSecHandle[IN/OUT]:Global data handle.  
        const WE_UCHAR *pucCertificateTypes[IN]:type of certificate.
        WE_INT32 iNbrCertificateTypes[IN]:number of type.
        St_SecTlsDistingubshedName *pstCertAuthorities[IN]:point to the issuer of user certificate.
        WE_INT32 iNumCertAuthorities[IN]:Number of issuer.
RETURN VALUE:
        none.    
USED GLOBAL VARIABLES:
        none.    
...
IMPORTANT NOTES:
        omit.
=====================================================================================*/
void Sec_iWimGetTLSUserCertNames(WE_HANDLE hSecHandle,const WE_UCHAR *pucCertificateTypes, 
                                 WE_INT32 iNbrCertificateTypes,
                                 St_SecTlsDistingubshedName *pstCertAuthorities, 
                                 WE_INT32 iNumCertAuthorities)
{ 
    WE_UCHAR   **ppucCertificateFriendlyNames = NULL;
    WE_INT32   iNbrOfCertificates = 0;
    WE_UINT16  *pusFriendlyNameLen = NULL;
    WE_INT16   *psCharSet = NULL;
    WE_INT8    *pcNameType = NULL;
    WE_INT32   *piCertificateIds = NULL;
    WE_INT32    iIndex=0;
    WE_INT32    iKLoop=0;
    WE_INT32    iJIndex=0;
    WE_INT32    iU=0; 
    ...
    E_WE_ERROR  eResult = E_WE_OK;
    WE_INT32    iRes=0;
    WE_UCHAR    *pucTmpFriendlyName=NULL;
    WE_UINT16   usTmpFriendlyNameLen=0;
    WE_UINT8    ucDoNotMatch=0;
    WE_INT8     *pcChainDepth = NULL;
    WE_UINT32   **ppuiCertChain = NULL;
    WE_UINT32   uiFileId=0;
    WE_UCHAR    *pucSubjectName=NULL;
    WE_INT32    iCheckRootCert = 0;
    WE_UINT32   uiIsOk=0;
    WE_INT16    sTmpCharSet=0;
    WE_CHAR     cTmpNameType=0;
    WE_UINT8    ucIsSameUCert=0;
    WE_INT32    iUIndex=0;
    WE_UINT8    ucNbrOfCaCert=0;    
    WE_INT32    iSearchLoop1=0;
    WE_INT32    iSearchLoop2=0;
    WE_INT32    iRootIndex=0;
    WE_LONG     lSize=0;
    WE_HANDLE   hFileHandle=NULL;
    WE_UCHAR    *pucUCertIdxFile=NULL;
    WE_INT32    iReadLen=0;
    WE_UINT8    ucUCertNum=0;
    WE_UINT8    *pucUCertDNHash=NULL;
    WE_INT32    iFind=0;
    WE_INT32    iTmpTestReturnNbrCert=0;
    WE_UINT32   *puiTmpBuf = NULL;
    WE_UINT8   *pucTmpBuf2 = NULL;
    WE_INT32    iLenTmpBuf2 =0; 

    St_SecWimTlsCaCertParamList *pstCaCertParamList=NULL;
    
    if(!hSecHandle||!pucCertificateTypes||!pstCertAuthorities)
    {
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_INVALID_PARAMETER, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        return ;
    }
    if (!Sec_WimIsInited(hSecHandle))
    {
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_WIM_NOT_INITIALISED, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        return; 
    }
    
    /*get user cert indexfile*/    
    eResult= WE_FILE_GETSIZE (SEC_WE_HANDLE,M_SEC_USERCERT_INDEXFILE_NAME,&lSize);    
    if (eResult!=E_WE_OK)                
    {
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        return; 
    }
    eResult = WE_FILE_OPEN(SEC_WE_HANDLE, M_SEC_USERCERT_INDEXFILE_NAME, WE_FILE_SET_RDONLY, &(hFileHandle)); 
    if (eResult != E_WE_OK)
    {   
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    ...
    pucUCertIdxFile=(WE_UCHAR *)WE_MALLOC((WE_UINT32)lSize*sizeof(WE_UCHAR));
    if(!pucUCertIdxFile)
    {
        eResult = WE_FILE_CLOSE(hFileHandle);
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_INSUFFICIENT_MEMORY, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        return; 
    }
    /*read the whole user cert indexfile*/
    eResult = WE_FILE_READ(hFileHandle, pucUCertIdxFile, lSize, &iReadLen);
    if ((eResult != E_WE_OK) || (iReadLen < 1))
    {
        M_SEC_FREE(pucUCertIdxFile);
        eResult = WE_FILE_CLOSE(hFileHandle);
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0,\
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        return; 
    ...
    ucUCertNum=*pucUCertIdxFile;
    if(!ucUCertNum)
    {
        M_SEC_FREE(pucUCertIdxFile);
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_OK, 0,\
            NULL,NULL,NULL,NULL,NULL,NULL,NULL);
        return ;
    }
    for(iIndex = 0;iIndex <M_SEC_USER_CERT_MAX_SIZE;iIndex++)
    {
        iLenTmpBuf2+=SEC_USER_CERT_KEYPAIRS[iIndex].iNameCount+1;
    }
    ppucCertificateFriendlyNames = (WE_UCHAR **)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_UCHAR *));
    pusFriendlyNameLen = (WE_UINT16 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_UINT16));
    psCharSet = (WE_INT16 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_INT16));
    pcNameType = (WE_INT8 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_INT8));
    piCertificateIds = (WE_INT32 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_INT32));
    pcChainDepth = (WE_INT8 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_INT8));
    ppuiCertChain = (WE_UINT32 **)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*sizeof(WE_UINT32 *));
    puiTmpBuf =  (WE_UINT32 *)WE_MALLOC(M_SEC_USER_CERT_MAX_SIZE*(3*sizeof(WE_UINT32)+1));
    pucTmpBuf2=  (WE_UINT8 *)WE_MALLOC((WE_UINT32)iLenTmpBuf2);
    
    if (!piCertificateIds||!psCharSet||!pusFriendlyNameLen||!pcNameType||!ppuiCertChain
        ||!pcChainDepth||!ppucCertificateFriendlyNames||!puiTmpBuf||!pucTmpBuf2)
    {
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_INSUFFICIENT_MEMORY, 0, \
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        if (psCharSet != NULL)
        {
            M_SEC_FREE(psCharSet);
        }
        if (pcNameType != NULL)
        {
            M_SEC_FREE(pcNameType);
        }
        if (pusFriendlyNameLen != NULL)
        {
            M_SEC_FREE(pusFriendlyNameLen);
        }
        if (piCertificateIds != NULL)
        {
            M_SEC_FREE(piCertificateIds);
        }
        if(ppuiCertChain!=NULL)
        {
            M_SEC_FREE(ppuiCertChain);
        }
        if(pcChainDepth!=NULL)
        {
            M_SEC_FREE(pcChainDepth);
        }
        if(puiTmpBuf!=NULL)
        {
            M_SEC_FREE(puiTmpBuf);
        }
        if(pucTmpBuf2!=NULL)
        {
            M_SEC_FREE(pucTmpBuf2);
        }
        if(ppucCertificateFriendlyNames!=NULL)
        {
           M_SEC_FREE(ppucCertificateFriendlyNames);
        }
        M_SEC_FREE(pucUCertIdxFile);
        return ;
    }
    (WE_VOID)WE_MEMSET(puiTmpBuf,0,M_SEC_USER_CERT_MAX_SIZE*(3*sizeof(WE_UINT32)+1));
    for(iIndex = 0;iIndex<M_SEC_USER_CERT_MAX_SIZE;iIndex++)
    {
        ppuiCertChain[iIndex] =puiTmpBuf+(WE_UINT32)iIndex*(3*sizeof(WE_UINT32)+1);
    }
    
    (WE_VOID)WE_MEMSET(pucTmpBuf2,0,(WE_UINT32)iLenTmpBuf2);
    
    for(iIndex = 0;iIndex<M_SEC_USER_CERT_MAX_SIZE;iIndex++)
    {
        WE_INT32 iFrom =0;
        for(iKLoop=0;iKLoop<iIndex;iKLoop++)
        {
            iFrom +=SEC_USER_CERT_KEYPAIRS[iIndex].iNameCount+1;
        }
        ppucCertificateFriendlyNames[iIndex] = pucTmpBuf2+iFrom;
    }
    /*get all x509 ca cert's subject,issuer,and length*/
    iRes=Sec_iWimGetTLSCaCertParamList(hSecHandle,&pstCaCertParamList,&ucNbrOfCaCert);
    if(iRes!=M_SEC_ERR_OK)
    {
        Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_INSUFFICIENT_MEMORY, 0, \
            NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    ...
    if (ucNbrOfCaCert != 0)
    {
        for (iIndex=0; iIndex<iNumCertAuthorities; iIndex++) 
        {
            iPos = 0;
            uiIsOk =0;
            iCheckRootCert = 0;
            /*search whether have root ca?*/        /*SWWIM_findRootCert()*/
            for(iSearchLoop1=0;iSearchLoop1<ucNbrOfCaCert;iSearchLoop1++)
            {
                if((pstCaCertParamList[iSearchLoop1].uiFileId>0)&&
                    (pstCaCertParamList[iSearchLoop1].iIsRootCert==SEC_ROOT_CERT)&&
                   (!WE_MEMCMP(pstCertAuthorities[iIndex].pucName,pstCaCertParamList[iSearchLoop1].pucIssuer,
                        pstCaCertParamList[iSearchLoop1].usIssuerLen)))
                {
                    iRootIndex =iSearchLoop1;/*get root ca index*/
                    uiIsOk = 1;
                    break;
                }
            }  /*Sec_WimFindRootCert*/      
            ...
            /*SWWIM_findSubjectToCAcert():  get pucSubjectName,pucUCertDNHash,uiFileId*/
            for(iSearchLoop2=0;iSearchLoop2<ucNbrOfCaCert;iSearchLoop2++)/*not root cert*/
            {
                if((pstCaCertParamList[iSearchLoop2].uiFileId>0)&&
                    (pstCaCertParamList[iSearchLoop2].iIsRootCert!=SEC_ROOT_CERT)&&
                    (pstCaCertParamList[iSearchLoop2].iIsX509OK==SEC_CERT_OK))
                {
                    if(!WE_MEMCMP(pstCertAuthorities[iIndex].pucName,
                        pstCaCertParamList[iSearchLoop2].pucIssuer,
                        pstCaCertParamList[iSearchLoop2].usIssuerLen))
                    {
                        pucSubjectName =pstCaCertParamList[iSearchLoop2].pucSubject; 
                        pucUCertDNHash =pstCaCertParamList[iSearchLoop2].aucSubjectHash;
                        uiFileId =pstCaCertParamList[iSearchLoop2].uiFileId;
                        uiIsOk=1;
                        break;
                    }
                }
            ...
            if (1==uiIsOk) 
            {  /*the depth at least 3*/
                ppuiCertChain[iNbrOfCertificates][iPos] = uiFileId;/*save the first CA cert's file id*/
                iPos++;
                pcChainDepth[iNbrOfCertificates] = (WE_INT8) iPos;/*save the cert chain depth iPos=1*/
                while (uiIsOk == 1) /*search into  cert chain*/
                {                    
                    /*SWWIM_findSubjectToCAcert():  get pucSubjectName,pucUCertDNHash,uiFileId*/
                   for(iSearchLoop1=0;iSearchLoop1<ucNbrOfCaCert;iSearchLoop1++)
                   {
                        ...
                        if((pstCaCertParamList[iSearchLoop1].uiFileId>0)&&
                           (pstCaCertParamList[iSearchLoop1].iIsRootCert!=SEC_ROOT_CERT)&&
                           (pstCaCertParamList[iSearchLoop1].iIsX509OK==SEC_CERT_OK))
                        {
                            if(!WE_MEMCMP(pucSubjectName,pstCaCertParamList[iSearchLoop1].pucIssuer,
                                pstCaCertParamList[iSearchLoop1].usIssuerLen) )  
                            {
                                pucSubjectName=pstCaCertParamList[iSearchLoop1].pucSubject; 
                                pucUCertDNHash=pstCaCertParamList[iSearchLoop1].aucSubjectHash;
                                uiFileId=pstCaCertParamList[iSearchLoop1].uiFileId;
                                uiIsOk=1;
                                break;
                            } 
                        }
                   ...
                   if (uiIsOk == 1)
                   {
                        ppuiCertChain[iNbrOfCertificates][iPos] = uiFileId;/*ca file id*/
                        iPos++;
                        pcChainDepth[iNbrOfCertificates] = (WE_INT8) iPos;/*depth*/
                   }
                }/*while ?*/
                uiIsOk = 1;
            }
            ...
            { 
                if(iCheckRootCert == 1)  /*have root ca cert*/
                {
                    pucSubjectName = pstCaCertParamList[iRootIndex].pucSubject;
                    pucUCertDNHash=pstCaCertParamList[iRootIndex].aucSubjectHash;
                    uiIsOk = 1;
                    pcChainDepth[iNbrOfCertificates] = 0;
                }
            }
            if (uiIsOk == 1)
            {    
                ucIsSameUCert = 0;
                for (iJIndex = 0; iJIndex<M_SEC_USER_CERT_MAX_SIZE; iJIndex++)
                {
                    ucDoNotMatch = 0;
                    if ((SEC_USER_CERT_KEYPAIRS[iJIndex].ucKeyUsage==M_SEC_PRIVATE_AUTHKEY) &&
                        (SEC_USER_CERT_KEYPAIRS[iJIndex].ucPublicKeyCert == 0))
                    {  
                        iFind=0;
                        iRes=Sec_iWimTLSFindCaDNHashToUCertDNHash(hSecHandle,
                            SEC_USER_CERT_KEYPAIRS[iJIndex].iUcertId,pucUCertDNHash,20,pucUCertIdxFile,lSize,&iFind);
                        if(iFind==1)
                        {
                             /*get user cert friendly name :which malloced in Sec_WimGetCertNameByID() */
                            iRes=Sec_WimTlsGetCertNameByID( hSecHandle,pucUCertIdxFile,\
                                            SEC_USER_CERT_KEYPAIRS[iJIndex].iUcertId,\
                                            &pucTmpFriendlyName, \
                                            &usTmpFriendlyNameLen, &sTmpCharSet,&cTmpNameType); 
                            if (!pucTmpFriendlyName||iRes!=M_SEC_ERR_OK)
                            {
                                Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0, 
                                                   NULL, NULL, NULL, NULL,NULL,NULL,NULL);
                                M_SEC_FREE(pucTmpBuf2);
                                M_SEC_FREE(ppucCertificateFriendlyNames);
                                if(pstCaCertParamList!=NULL)
                                {
                                    for(iSearchLoop1=0;iSearchLoop1<ucNbrOfCaCert;iSearchLoop1++)
                                    {   
                                        if((pstCaCertParamList[iSearchLoop1].uiFileId>0)&&
                                            (pstCaCertParamList[iSearchLoop1].iIsX509OK==1))
                                        {
                                            if(pstCaCertParamList[iSearchLoop1].pucSubject)
                                            {
                                                M_SEC_FREE(pstCaCertParamList[iSearchLoop1].pucSubject);
                                            }
                                            if(pstCaCertParamList[iSearchLoop1].pucIssuer)
                                            {
                                                M_SEC_FREE(pstCaCertParamList[iSearchLoop1].pucIssuer);
                                            }
                                        }         
                                    }
                                    M_SEC_FREE(pstCaCertParamList);
                                }
                                if(piCertificateIds)
                                {
                                    M_SEC_FREE(piCertificateIds);
                                ...
                                M_SEC_FREE(pusFriendlyNameLen);
                                if (pucTmpFriendlyName)
                                {
                                    M_SEC_FREE(pucTmpFriendlyName);
                                }
                                if (pcChainDepth != NULL)
                                {
                                    M_SEC_FREE(pcChainDepth);
                            ...
                            ucDoNotMatch = 1;
                            for (iU =0; iU<iNbrCertificateTypes; iU++)
                            {
                                if (M_SEC_SP_RSA == pucCertificateTypes[iU])
                                {
                                    ucDoNotMatch = 0;
                                    iU = iNbrCertificateTypes;
                                }
                            }
                            if (ucDoNotMatch == 0)      
                            {
                                psCharSet[iNbrOfCertificates] = sTmpCharSet;
                                pusFriendlyNameLen[iNbrOfCertificates] = usTmpFriendlyNameLen;
                                for (iKLoop =0;iKLoop<pusFriendlyNameLen[iNbrOfCertificates];iKLoop++)
                                {
                                    ppucCertificateFriendlyNames[iNbrOfCertificates][iKLoop] = pucTmpFriendlyName[iKLoop];
                                }
                                M_SEC_FREE(pucTmpFriendlyName);
                                 
                                ppucCertificateFriendlyNames[iNbrOfCertificates][iKLoop] = '\0';
                                pcNameType[iNbrOfCertificates] = SEC_USER_CERT_KEYPAIRS[iJIndex].cNameType;
                                piCertificateIds[iNbrOfCertificates] = SEC_USER_CERT_KEYPAIRS[iJIndex].iUcertId;
                                if(1==ucIsSameUCert)
                                {                            
                                    for(iUIndex=0;iUIndex<pcChainDepth[iNbrOfCertificates-1];iUIndex++)
                                    {
                                        ppuiCertChain[iNbrOfCertificates][iUIndex] =ppuiCertChain[iNbrOfCertificates-1][iUIndex];
                                    }
                                    pcChainDepth[iNbrOfCertificates] = pcChainDepth[iNbrOfCertificates-1];
                                }
                                ucIsSameUCert =1;
                                iNbrOfCertificates++;
                                if(pucTmpCert)
                                {
                                    M_SEC_FREE(pucTmpCert);
                                }
                            }
                            else
                            { 
    ...
    //start add by tang for tlsue
    if(iNbrOfCertificates>=1)
    {
        iTmpTestReturnNbrCert=1;
    }//end 

    Sec_WimTlsGetUserCertNamesResponse(hSecHandle,M_SEC_ERR_OK, iTmpTestReturnNbrCert, \
                                (const void *const *)ppucCertificateFriendlyNames,\
                                pusFriendlyNameLen, psCharSet, pcNameType,\
                                (const WE_INT32 *)piCertificateIds, pcChainDepth, ppuiCertChain);
...
/*=====================================================================================
FUNCTION: 
        Sec_iWimGetChainOfCertificates
CREATE DATE: 
        2007-01-19
AUTHOR: 
        tang
...
ARGUMENTS PASSED:
        hSecHandle[IN]: Global data handle.
        ucChainDepth[IN]:  depth of cert chain.
        puiCertChainIds[IN]:   pointer to the cert file id.
RETURN VALUE:
        none.    
USED GLOBAL VARIABLES:
        none.    
...
IMPORTANT NOTES:
        omit.
=====================================================================================*/
void Sec_iWimGetChainOfCertificates(WE_HANDLE hSecHandle,WE_UINT8 ucChainDepth,
                         WE_UINT32 *puiCertChainIds)
{
  WE_UCHAR      **ppucCert=NULL;
  WE_UINT16     *pusCertLen=NULL;
  WE_INT32      iIIndex=0;
  WE_INT32      iLIndex=0;
  WE_INT32      iKLoop=0;
  WE_UINT32     uiFileId = 0;                
  ...
  if(NULL==hSecHandle||NULL==puiCertChainIds||!ucChainDepth)
  {
      Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_INVALID_PARAMETER, 0, NULL, NULL);
      return;
  }

  if (!Sec_WimIsInited(hSecHandle))
  {
      Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_WIM_NOT_INITIALISED, 0, NULL, 0);
      return;
  }
     /* get size of indexfile.*/
  eRes = WE_FILE_GETSIZE(SEC_WE_HANDLE, M_SEC_CA_CERT_INDEXFILE_NAME, &(iIndexFileSize));
  if (E_WE_OK != eRes)
  {
     Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, ucChainDepth, NULL, 0);
     return;
  }
  pucIndexFile = (WE_UCHAR *)WE_MALLOC((WE_UINT32)iIndexFileSize * sizeof(WE_UCHAR));  
  if (NULL == pucIndexFile)
  {
     Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_INSUFFICIENT_MEMORY, 0, NULL, 0);
     return;
  }
    
  /* open indexfile */
  eRes = WE_FILE_OPEN(SEC_WE_HANDLE, M_SEC_CA_CERT_INDEXFILE_NAME, WE_FILE_SET_RDONLY, &(hFileHandle));
  if (eRes != E_WE_OK)
  {
     WE_FREE(pucIndexFile);
     Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0, NULL, 0);
     return;
  }    
  /* read the whole ca indexfile*/
  eRes = WE_FILE_READ(hFileHandle, pucIndexFile, iIndexFileSize, &(iReadLen));
  if ((eRes!=E_WE_OK) || (iReadLen<iIndexFileSize))
  {
     eRes = WE_FILE_CLOSE(hFileHandle);
     WE_FREE(pucIndexFile);
     Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, 0, NULL, 0);
  ...
  eRes = WE_FILE_CLOSE(hFileHandle);
  /*get ca number*/
  pucTmpIndexFile=pucIndexFile;
  ucNbrOfCaCert=*pucTmpIndexFile; 
  pucTmpIndexFile++;
  if (0==ucNbrOfCaCert)
  {
     Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_OK, 0, NULL, 0);
     return;
  }  
  ppucCert = (WE_UCHAR **)WE_MALLOC((ucChainDepth+1)*sizeof(WE_UCHAR *));
  pusCertLen = (WE_UINT16 *)WE_MALLOC((ucChainDepth+1)*sizeof(WE_UINT16));
  for (iLIndex=0; iLIndex<ucChainDepth; iLIndex++)
  {
    for (iIIndex=0; iIIndex<ucNbrOfCaCert; iIIndex++)      
    {
      /*get cert type*/
      pucTmpIndexFile+=1;
      /*get file id*/
      Sec_StoreStrUint8to32(pucTmpIndexFile, &uiFileId); 
      pucTmpIndexFile+=4;
      pucTmpIndexFile+=4;
      pucTmpIndexFile+=20+1+2+1;
      /*get length of friendly name */
      Sec_StoreStrUint8to16(pucTmpIndexFile, &usFrinedlyNameLen);
      pucTmpIndexFile+=2;
      pucTmpIndexFile+=usFrinedlyNameLen;     
      
     if (puiCertChainIds[iLIndex] == uiFileId)      
     {
          /*get ca cert and cert length*/
          iRes=Sec_WimGetCertByID( hSecHandle, (WE_INT32)uiFileId, &ppucCert[iLIndex], &pusCertLen[iLIndex]);

          if (M_SEC_ERR_OK!=iRes)
          {
            Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_GENERAL_ERROR, ucChainDepth,NULL, 0);
            for(iKLoop=0;iKLoop<iLIndex;iKLoop++)
            {
                WE_FREE(ppucCert[iLIndex]);
            }    
            WE_FREE(ppucCert);
            WE_FREE(pusCertLen);
            return;
  ...
  Sec_WimGetChainOfCertificatesResponse(hSecHandle,M_SEC_ERR_OK, ucChainDepth, ppucCert, pusCertLen);
  for (iIIndex=0; iIIndex<ucChainDepth; iIIndex++)
  {
      WE_FREE(ppucCert[iIIndex]);
  }
  WE_FREE(pusCertLen);
  WE_FREE(ppucCert);
}
                            ...
                            St_SecTlsKeyExchangeParams *pstParam, const WE_UCHAR *pucRandval)
{
    ...
    if (!Sec_WimIsInited(hSecHandle))
    {
        Sec_WimKeyExchangeResponse(hSecHandle,iTargetID, M_SEC_ERR_WIM_NOT_INITIALISED, 0, NULL, 0);
        return;
    }
    ...
    case  E_SEC_TLS_KEYEXCH_RSA_EXPORT      : 
        {
         ...
         else
         {
             ...
             else if ((*pstParam->stSecCert.pucBuf) == 48) /*x509 cert,and get certificate public key*/           
             {
         ...
         pucMasterSecret = (WE_CHAR *)WE_MALLOC (M_SEC_TLS_MASTER_SECRET_LEN);
...
IMPORTANT NOTES:
        omit.
=====================================================================================*/
WE_VOID Sec_iWimGetMasterSecret (WE_HANDLE hSecHandle, WE_INT32 iMasterSecretID)
{
    ...
    if (!Sec_WimIsInited(hSecHandle))
    {
        Sec_SSLGetMasterSecretResp( hSecHandle, 0, M_SEC_ERR_WIM_NOT_INITIALISED,
                             aucMasterSecret);
        return ;
    }
    ...
    if (iRes != M_SEC_ERR_OK)
    {
        Sec_SSLGetMasterSecretResp( hSecHandle, 0, iRes,aucMasterSecret);
        return ;
    }
    ...
    //return Sec_LibHashUpdate (hSecHandle,aucMasterSecret,M_SEC_TLS_MASTER_SECRET_LEN,hHashHandle);
}
/*=====================================================================================
FUNCTION: 
        Sec_iWimGetTLSCaCertParamList
CREATE DATE: 
        2007-01-19
AUTHOR: 
        tang
DESCRIPTION:
        get ca cert parameter. 
ARGUMENTS PASSED:
        WE_HANDLE hSecHandle[IN]: Global data handle.
        St_SecWimTlsCaCertParamList **ppstCaCertParamList[OUT]:pointer to the structue of St_SecWimTlsCaCertParamList.
        WE_UINT8 *pucNumCaCert[OUT]:number of certificate.
RETURN VALUE:
        none.    
USED GLOBAL VARIABLES:
        none.    
                                          ...
                                          WE_UINT8 *pucNumCaCert)
{
    ...
    St_SecCertificate stCert={0};
    WE_INT32    iKLoop=0;
    WE_INT32   iSubjectHashLen=20;
    E_WE_ERROR eRes = E_WE_OK;
    
    
    if(!ppstCaCertParamList||!hSecHandle||!pucNumCaCert)
    {
        return M_SEC_ERR_INVALID_PARAMETER;
    }
    ...
    eRes = WE_FILE_READ(hFileHandle, &ucNbrOfCaCert, 1, &(iReadLen));
    if ((eRes != E_WE_OK) || (iReadLen < 1))
    {
        eRes = WE_FILE_CLOSE(hFileHandle);
        return M_SEC_ERR_GENERAL_ERROR;
    }
    ...
    *pucNumCaCert=ucNbrOfCaCert;
    if(0==ucNbrOfCaCert)
    {      
       return M_SEC_ERR_OK;
    }
    
    *ppstCaCertParamList=(St_SecWimTlsCaCertParamList *)WE_MALLOC(ucNbrOfCaCert*sizeof(St_SecWimTlsCaCertParamList));
    (WE_VOID)WE_MEMSET(*ppstCaCertParamList,0,ucNbrOfCaCert*sizeof(St_SecWimTlsCaCertParamList));
    if(NULL==*ppstCaCertParamList)
    {
        return M_SEC_ERR_INSUFFICIENT_MEMORY;
    }
    for(iLoop=0;iLoop<ucNbrOfCaCert;iLoop++)
    {
        iRes = Sec_WimGetCACert(hSecHandle,iLoop+1, &pucRootCert,&usRootCertLen,\
                                &uiFileId,&ucCertType, &ucAble);
        if (iRes != M_SEC_ERR_OK)
        {
            if (pucRootCert != NULL)
            {
            ...
            {
                if(((*ppstCaCertParamList)[iLoop]).pucIssuer)
                {
                    WE_FREE(((*ppstCaCertParamList)[iLoop]).pucIssuer);
                }
                if(((*ppstCaCertParamList)[iLoop]).pucSubject)
                {
                    WE_FREE(((*ppstCaCertParamList)[iLoop]).pucSubject);
                }
            }
            WE_FREE(*ppstCaCertParamList);
            *ppstCaCertParamList=NULL;
            return M_SEC_ERR_GENERAL_ERROR;
        }
        usRootCertLen = 0;
        
        ((*ppstCaCertParamList)[iLoop]).uiFileId=uiFileId;
        
        if (M_SEC_X509_CA_CERT==ucCertType)     
        {
            iRes = Sec_X509CertParse(hSecHandle,pucRootCert, &usRootCertLen, &stCert);            
            ((*ppstCaCertParamList)[iLoop]).iIsRootCert=SEC_CA_CERT;
        }
        else
        {
            if (pucRootCert != NULL)
        ...
        if ((uiNow > stCert.iValidNotBefore) && (stCert.iValidNotAfter > uiNow)) 
        {
            ((*ppstCaCertParamList)[iLoop]).usIssuerLen=stCert.stCertUnion.stX509.usIssuerLen;
            ((*ppstCaCertParamList)[iLoop]).usSubjectLen=stCert.stCertUnion.stX509.usSubjectLen ;
            ((*ppstCaCertParamList)[iLoop]).pucIssuer=(WE_UINT8*)WE_MALLOC(((*ppstCaCertParamList)[iLoop]).usIssuerLen*sizeof(WE_UINT8));
            ((*ppstCaCertParamList)[iLoop]).pucSubject=(WE_UINT8*)WE_MALLOC(((*ppstCaCertParamList)[iLoop]).usSubjectLen*sizeof(WE_UINT8));
            if((NULL==((*ppstCaCertParamList)[iLoop]).pucIssuer)||
                (NULL==((*ppstCaCertParamList)[iLoop]).pucSubject))
            {
                for(iKLoop=0;iKLoop<iLoop;iKLoop++)
                {
                    if(((*ppstCaCertParamList)[iLoop]).pucIssuer)
            ...
            (WE_VOID)WE_MEMCPY(((*ppstCaCertParamList)[iLoop]).pucIssuer,
                (const WE_CHAR *)stCert.pucIssuer,((*ppstCaCertParamList)[iLoop]).usIssuerLen);
            (WE_VOID)WE_MEMCPY(((*ppstCaCertParamList)[iLoop]).pucSubject,
                (const WE_CHAR *)stCert.pucSubject,((*ppstCaCertParamList)[iLoop]).usSubjectLen);
                
            iRes = Sec_LibHash(hSecHandle, E_SEC_ALG_HASH_SHA1, ((*ppstCaCertParamList)[iLoop]).pucSubject,
                            ((*ppstCaCertParamList)[iLoop]).usSubjectLen, 
                            ((*ppstCaCertParamList)[iLoop]).aucSubjectHash, &iSubjectHashLen);
            if(iRes!= M_SEC_ERR_OK)
            {
          	    for(iKLoop=0;iKLoop<iLoop;iKLoop++)
                {
            ...
            ((*ppstCaCertParamList)[iLoop]).iIsRootCert=0;
            ((*ppstCaCertParamList)[iLoop]).iIsX509OK=SEC_CERT_OK;
            if(((*ppstCaCertParamList)[iLoop]).usIssuerLen==((*ppstCaCertParamList)[iLoop]).usSubjectLen)
            {
                if (!WE_MEMCMP((const WE_CHAR *)stCert.pucIssuer, (const WE_CHAR *)stCert.pucSubject ,
                    stCert.stCertUnion.stX509.usIssuerLen))
                {
                    ((*ppstCaCertParamList)[iLoop]).iIsRootCert=SEC_ROOT_CERT;
                }  
            }
        }
        if (pucRootCert != NULL)
...
/*=====================================================================================
FUNCTION: 
        Sec_iWimTLSFindCaDNHashToUCertDNHash
CREATE DATE: 
        2007-01-19
AUTHOR: 
        tang
...
ARGUMENTS PASSED:
        WE_HANDLE hSecHandle[IN]: Global data handle.
        WE_INT32 iFileId[IN]: file id.
        const WE_UCHAR *pucCaDNHash[IN]:dn hash.
        WE_INT32 iCaDNHashLen[IN]:length of hash.
        const WE_UCHAR *pucUCertIdxFile[IN]:pointer to the user cert indexfile.
        WE_INT32 iCertIdxFileLen[IN]: length of indexfile.
        WE_INT32 *piFind[IN]:if match.is 1.else is 0.
RETURN VALUE:
        none.    
USED GLOBAL VARIABLES:
        none.    
...
static WE_INT32 Sec_iWimTLSFindCaDNHashToUCertDNHash(WE_HANDLE hSecHandle,WE_INT32 iFileId,
                                             ...
                                             WE_INT32 *piFind)
{
    WE_UCHAR *pucIndexFile=(WE_UCHAR *)pucUCertIdxFile;
    WE_UCHAR *pucDnHash=(WE_UCHAR *)pucCaDNHash;
    WE_INT32  iNumInIndexFile = 0;
    WE_INT32  iTmpCertId = 0;
    WE_INT32  iIndexI = 0;
    WE_UINT16 usNameLen=0;
    
    if(!hSecHandle||!pucDnHash||(iCaDNHashLen!=20)||!piFind||!pucIndexFile)
    {
        return M_SEC_ERR_INVALID_PARAMETER;
    }
    iCertIdxFileLen=iCertIdxFileLen;
    if ((iFileId<M_SEC_USER_CERT_ID_START) ||(iFileId>=M_SEC_CA_CERT_ID_START))
    {
        return M_SEC_ERR_INVALID_PARAMETER;
    }
      
    *piFind=0;
    iNumInIndexFile = *pucIndexFile;
    pucIndexFile++;
    for (iIndexI=0; iIndexI<iNumInIndexFile; iIndexI++) 
    {
        pucIndexFile++;
        Sec_StoreStrUint8to32(pucIndexFile, (WE_UINT32 *)(&iTmpCertId));
        pucIndexFile += 4+4;
        if (iTmpCertId == iFileId)
        {
            if(!WE_MEMCMP(pucIndexFile,pucDnHash,20))
            {
                *piFind=1;
                return M_SEC_ERR_OK;
            }
        }
        pucIndexFile+=20+20+1+2;
        Sec_StoreStrUint8to16(pucIndexFile, &usNameLen);
        pucIndexFile += 2+usNameLen; 
    }    
    return M_SEC_ERR_OK;
}
/*=====================================================================================
FUNCTION: 
        Sec_WimTlsGetCertNameByID
CREATE DATE: 
        2007-01-19
AUTHOR: 
        tang
...
ARGUMENTS PASSED:
        WE_HANDLE hSecHandle[IN]: Global data handle.
        const WE_UCHAR *pucCertIdxFile[IN]:pointer to the user cert indexfile.
        WE_INT32 iCertId[IN]:cert id.
        WE_UINT8 **ppucCertNames[OUT]:
        WE_UINT16 *pusNameLen[OUT]:
        WE_INT16 *psCharSet[OUT]:
        WE_INT8 *pcNameType[OUT]:
RETURN VALUE:
        none.    
USED GLOBAL VARIABLES:
        none.    
...
WE_INT32 Sec_WimTlsGetCertNameByID(WE_HANDLE hSecHandle,    
                                ...
                                WE_INT8 *pcNameType)
{
    WE_UCHAR  *pucIndexFile = (WE_UCHAR *)pucCertIdxFile;
    WE_INT32  iNumInIndexFile = 0;
    WE_INT32  iTmpCertId = 0;
    WE_INT32  iIndexI = 0;

    if (!hSecHandle ||!ppucCertNames || !pcNameType ||!pusNameLen||!pucIndexFile)
    {
        return M_SEC_ERR_INVALID_PARAMETER;
    } 
    ...
    for (iIndexI=0; iIndexI<iNumInIndexFile; iIndexI++) 
    {
        ...
        if (iTmpCertId == iCertId)
        {            
            pucIndexFile += 4+4+20+20;   
            *pcNameType = (WE_CHAR)(*pucIndexFile);
            pucIndexFile++;
            Sec_StoreStrUint8to16(pucIndexFile, (WE_UINT16 *)psCharSet);
            pucIndexFile += 2;        
             Sec_StoreStrUint8to16(pucIndexFile, pusNameLen);
            pucIndexFile += 2;        
            *ppucCertNames = (WE_UCHAR *)WE_MALLOC((*pusNameLen) * sizeof(WE_UCHAR)); 
            if ((NULL==(*ppucCertNames)) && (0!=(*pusNameLen)))
            {
            ...
            (WE_VOID)WE_MEMCPY(*ppucCertNames, pucIndexFile, *pusNameLen);        
            return M_SEC_ERR_OK;
        }
        else
        {
            pucIndexFile +=4+4+20+20+1+2 ;
            Sec_StoreStrUint8to16(pucIndexFile, pusNameLen);
            pucIndexFile+=2+(*pusNameLen);
        }
    }
    return M_SEC_ERR_NOT_FOUND;    
}
