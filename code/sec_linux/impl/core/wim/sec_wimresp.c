                                 ...
                                 const WE_UCHAR* pucPublicValue, WE_INT32 iPublicValueLen)
{
    ...
    WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,
      "Sec_WimKeyExchangeResponse(iTargetID = %d, iResult = %d, masterSecretId = %d, publicValueLen = %d)\n",\
...
WE_VOID Sec_WimTlsGetUserCertNamesResponse(WE_HANDLE hSecHandle,WE_INT32 iResult, WE_INT32 iNumberOfCertificates, 
                                      ...
                                      WE_UINT16 *pusFriendlyNameLen, WE_INT16 *psCharSet, 
                                      WE_INT8 *pcNameType, const WE_INT32 *piCertificateIds,
                                      WE_INT8 *pcChainDepth, WE_UINT32 **ppuiCertChainIds)
{
    St_SecTlConnCertNamesResp *pstP=NULL;
    WE_INT32                  iIIndex=0;
    WE_INT32                  iJIndex=0;
    WE_CHAR                   **ppcQ = (WE_CHAR **)pvCertificateFriendlyNames;
    WE_CHAR                   *pcTmpName=NULL;
    ...
    WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,
        "Sec_WimTlsGetUserCertNamesResponse(result = %d, iNumberOfCertificates = %d)\n",\
        iResult, iNumberOfCertificates));    
    ...
    M_SEC_WAITING_FOR_WIM_RESULT = 0;
    M_SEC_WANTS_TO_RUN = 1;
    pstP = (St_SecTlConnCertNamesResp *)WE_MALLOC(sizeof(St_SecTlConnCertNamesResp));
    if (!pstP)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    ...
    pstP->eType = E_SEC_DP_GET_CONN_CERT_NAMES; 
    if ((pstP->iResult = iResult) == M_SEC_ERR_WIM_NOT_INITIALISED)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = ( St_SecTlWimFunctionId *)pstP;
        return;
    }
    if (iNumberOfCertificates == 0)
    { 
        pstP->iNbrOfCerts = iNumberOfCertificates;
        pstP->pstCertNames = NULL;
        pstP->pcChainDepth = NULL;
        pstP->ppuiCertChainIds = NULL;
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = ( St_SecTlWimFunctionId *)pstP;
        return;
    }
    if((pstP->iResult != M_SEC_ERR_OK) && (piCertificateIds == NULL))
    {
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    }
    pstP->pstCertNames = (St_SecCertName *)WE_MALLOC((WE_UINT32)iNumberOfCertificates*sizeof(St_SecCertName));
    if (!pstP->pstCertNames)
    {
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    ...
    if ((pcChainDepth != NULL) && (ppuiCertChainIds != NULL))
    {
        pstP->ppuiCertChainIds = (WE_UINT32 **)WE_MALLOC((WE_UINT32)iNumberOfCertificates*sizeof(WE_UINT32 *));
        pstP->pcChainDepth = (WE_INT8 *)WE_MALLOC((WE_UINT32)iNumberOfCertificates*sizeof(WE_INT8));
        if(!pstP->ppuiCertChainIds||!pstP->pcChainDepth)
        {
            if(pstP->ppuiCertChainIds)
            {
                WE_FREE(pstP->ppuiCertChainIds);
            }
            if(pstP->pcChainDepth)
            {
                WE_FREE(pstP->pcChainDepth);
            }
            WE_FREE(pstP->pstCertNames);
            WE_FREE(pstP);
            M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
            return;
        }
    ...
    for (iIIndex=0; iIIndex<iNumberOfCertificates; iIIndex++)
    {
        if(!pusFriendlyNameLen||!ppcQ||!piCertificateIds||!psCharSet||!pcNameType)
        {
            if(pstP->ppuiCertChainIds)
            {
                WE_FREE(pstP->ppuiCertChainIds);
        ...
        pstP->pstCertNames[iIIndex].pvFriendlyName = (WE_CHAR *)WE_MALLOC(((WE_UINT32)pusFriendlyNameLen[iIIndex]+1));

        pcTmpName = pstP->pstCertNames[iIIndex].pvFriendlyName;
        for (iJIndex=0; iJIndex<pusFriendlyNameLen[iIIndex]; iJIndex++)
        {
           pcTmpName[iJIndex] = ppcQ[iIIndex][iJIndex];
        }
        pcTmpName[iJIndex] = '\0';
        
        pstP->pstCertNames[iIIndex].usFriendlyNameLen = pusFriendlyNameLen[iIIndex];
        pstP->pstCertNames[iIIndex].iCertId = piCertificateIds[iIIndex];
        pstP->pstCertNames[iIIndex].sCharSet = psCharSet[iIIndex];
        pstP->pstCertNames[iIIndex].cNameType = pcNameType[iIIndex];

        if ((pcChainDepth != NULL) && (ppuiCertChainIds != NULL))
        {
            pstP->pcChainDepth[iIIndex] = pcChainDepth[iIIndex];
            if (pcChainDepth[iIIndex] != 0)
            {
                pstP->ppuiCertChainIds[iIIndex] = (WE_UINT32 *)WE_MALLOC(((WE_UINT32)((WE_INT32)pcChainDepth[iIIndex]+1) *sizeof(WE_UINT32)));
                for (iJIndex=0; iJIndex<pcChainDepth[iIIndex]; iJIndex++)
                {
                   pstP->ppuiCertChainIds[iIIndex][iJIndex] = ppuiCertChainIds[iIIndex][iJIndex];
                }
            }
        }
    }
...
{
    St_SecTlsGetChainOfCertsResp *pstP=NULL;
    WE_INT32 iIIndex=0;
    WE_INT32 iJIndex=0;
    WE_INT32 iKLoop=0;
    WE_UCHAR **ppucQ = (WE_UCHAR **)ppucCerts;
    ...
    /*
    WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,
        "Sec_WimGetChainOfCertificatesResponse(iResult = %d, numberOfCertificates = %d)\n",\
        iResult, iNbrOfCerts));
    
    if ((iNbrOfCerts > 0))
    {
        WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,"ppucCerts[0] = %d", ppucCerts[0]));
        if (pusCertLens[0] > 0)
        {    
            WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0, "ppucCerts[0] =\n"));
            WE_LOG_DATA(WE_LOG_DETAIL_MEDIUM, 0, ppucCerts[0],pusCertLens[0]);
        }
    }
    else
    {
        WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,  "ppucCerts[0] = NULL"));
    }*/
    ...
    M_SEC_WAITING_FOR_WIM_RESULT = 0;
    M_SEC_WANTS_TO_RUN = 1;
    pstP = ( St_SecTlsGetChainOfCertsResp *)WE_MALLOC(sizeof(St_SecTlsGetChainOfCertsResp));
    if (!pstP)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    ...
    pstP->eType = E_SEC_DP_GET_CERT_CHAIN; 
    if ((pstP->iResult = iResult) == M_SEC_ERR_WIM_NOT_INITIALISED)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = ( St_SecTlWimFunctionId *)pstP;
        return;
    ...
    if (iNbrOfCerts == 0)
    { 
        pstP->iNbrOfCerts = iNbrOfCerts;
        pstP->ppucCerts = NULL;
        pstP->pusCertLen = NULL;
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = ( St_SecTlWimFunctionId *)pstP;
        return;
    }
    if((pstP->iResult != M_SEC_ERR_OK) && (ppucCerts == NULL))
    {
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    }
    pstP->ppucCerts = (WE_UCHAR **)WE_MALLOC((WE_UINT32)iNbrOfCerts*sizeof(WE_UCHAR *));
    if (!pstP->ppucCerts)
    {
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
    ...
    pstP->pusCertLen = (WE_UINT16 *)WE_MALLOC((WE_UINT32)iNbrOfCerts*sizeof(WE_UINT16));
    if(!pstP->pusCertLen)
    {
        WE_FREE(pstP->ppucCerts);
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    }
    for (iIIndex=0; iIIndex<iNbrOfCerts; iIIndex++)
    {
        if(!pusCertLens||!ppucQ)
        {
            WE_FREE(pstP->ppucCerts);
            WE_FREE(pstP->pusCertLen);
            WE_FREE(pstP);
            M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
            return;
        }
        pstP->ppucCerts[iIIndex] = (WE_UCHAR *)WE_MALLOC((pusCertLens[iIIndex]+1)*sizeof(WE_UCHAR));
        if(!pstP->ppucCerts[iIIndex])
        {
            for(iKLoop=0;iKLoop<iIIndex;iKLoop++)
            {
                WE_FREE(pstP->ppucCerts[iKLoop]);
            }
            WE_FREE(pstP->ppucCerts);
            WE_FREE(pstP->pusCertLen);
            WE_FREE(pstP);
        ...
        pucTmpName = pstP->ppucCerts[iIIndex];
        for (iJIndex=0; iJIndex<pusCertLens[iIIndex]; iJIndex++)
        {
            pucTmpName[iJIndex] = ppucQ[iIIndex][iJIndex];
        }
        pucTmpName[iJIndex] = '\0';
        pstP->pusCertLen[iIIndex] = pusCertLens[iIIndex];
    }
    pstP->iNbrOfCerts = iNbrOfCerts;
...
{
    WE_INT32 iIIndex=0;
    St_SecTlsGetCiphersuitesResp *pstP=NULL;
    ...
    WE_LOG_MSG((WE_LOG_DETAIL_MEDIUM, 0,
        "Sec_WimGetCipherSuitesResponse(iTargetID = %d, result = %d, iCipherSuitesLen = %d)\n",\
    ...
    M_SEC_WAITING_FOR_WIM_RESULT = 0;
    M_SEC_WANTS_TO_RUN = 1;
    pstP = (St_SecTlsGetCiphersuitesResp *)WE_MALLOC(sizeof( St_SecTlsGetCiphersuitesResp));
    if (!pstP)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        WE_FREE(pstP);
    ...
    if (pucCipherSuites)
    {
        pstP->iCipherSuitesLen = iCipherSuitesLen;
    }
    else 
    {
        pstP->iCipherSuitesLen = 0;
    }
    if (pstP->iCipherSuitesLen == 0)
    {
        pstP->pucCipherSuites = NULL;
    }
    if(0==iCipherSuitesLen||!pucCipherSuites)
    {
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        WE_FREE(pstP);
        return;
    }
    pstP->pucCipherSuites = (WE_UCHAR *)WE_MALLOC((WE_UINT32)iCipherSuitesLen*sizeof(WE_UCHAR));
    if (!(pstP->pucCipherSuites))
    {
        WE_FREE(pstP);
        M_SEC_CURRENT_WIM_RESPONSE_VALUES = NULL;
        return;
    }
    pstP->eType = E_SEC_DP_GET_TLS_CIPHERSUITE;
    pstP->iTargetID = iTargetID;
    pstP->iResult = iResult;
    for (iIIndex=0; iIIndex<iCipherSuitesLen; iIIndex++)
    {
        pstP->pucCipherSuites[iIIndex] = pucCipherSuites[iIIndex];
    }
    M_SEC_CURRENT_WIM_RESPONSE_VALUES = ( St_SecTlWimFunctionId *)pstP;
}
